...
@section('content')
    @php
        ...
        if (! in_array($activeTab, ['admin-overview', 'admin-courses', 'admin-users'], true)) {
    ...
    <section class="container dashboard-section" aria-labelledby="admin-title">
        ...
        <div class="admin-tabs" role="tablist" aria-label="{{ __('Secciones de administración') }}">
            <button class="admin-tab {{ $activeTab === 'admin-overview' ? 'is-active' : '' }}" id="tab-admin-overview" type="button" role="tab" tabindex="{{ $activeTab === 'admin-overview' ? '0' : '-1' }}" aria-selected="{{ $activeTab === 'admin-overview' ? 'true' : 'false' }}" aria-controls="admin-overview" data-admin-tab="admin-overview">{{ __('Resumen') }}</button>
            <button class="admin-tab {{ $activeTab === 'admin-courses' ? 'is-active' : '' }}" id="tab-admin-courses" type="button" role="tab" tabindex="{{ $activeTab === 'admin-courses' ? '0' : '-1' }}" aria-selected="{{ $activeTab === 'admin-courses' ? 'true' : 'false' }}" aria-controls="admin-courses" data-admin-tab="admin-courses">{{ __('Cursos') }}</button>
            <button class="admin-tab {{ $activeTab === 'admin-users' ? 'is-active' : '' }}" id="tab-admin-users" type="button" role="tab" tabindex="{{ $activeTab === 'admin-users' ? '0' : '-1' }}" aria-selected="{{ $activeTab === 'admin-users' ? 'true' : 'false' }}" aria-controls="admin-users" data-admin-tab="admin-users">{{ __('Alumnos') }}</button>
        </div>
        <div id="admin-overview" class="admin-panel {{ $activeTab === 'admin-overview' ? 'is-active' : '' }}" role="tabpanel" aria-labelledby="tab-admin-overview" tabindex="0" @if($activeTab !== 'admin-overview') hidden @endif>
        ...
        <div id="admin-courses" class="admin-panel {{ $activeTab === 'admin-courses' ? 'is-active' : '' }}" role="tabpanel" aria-labelledby="tab-admin-courses" tabindex="0" @if($activeTab !== 'admin-courses') hidden @endif>
            ...
            <div class="admin-record-grid">
                @forelse($courses as $course)
                    <article class="record-card">
                        <div><span class="preview-label">{{ __($course->category) }} · {{ __($course->level) }}</span><h3>{{ $course->title }}</h3><p>{{ Str::limit($course->description, 100) }}</p></div>
        ...
        <div id="admin-users" class="admin-panel {{ $activeTab === 'admin-users' ? 'is-active' : '' }}" role="tabpanel" aria-labelledby="tab-admin-users" tabindex="0" @if($activeTab !== 'admin-users') hidden @endif>
                    ...
                    <tbody>
                        @forelse($users as $user)
                            <tr>
                                ...
                                <td><span class="role-badge {{ $user->is_admin ? 'role-admin' : '' }}">{{ $user->is_admin ? __('Administrador') : __('Alumno') }}</span><details><summary>{{ __('Editar ficha') }}</summary><form method="POST" action="{{ route('admin.users.update', $user) }}" class="user-edit-form"><input type="hidden" name="_admin_tab" value="admin-users">@csrf @method('PUT')<input name="name" value="{{ $user->name }}" aria-label="{{ __('Nombre') }}" required><input name="email" type="email" value="{{ $user->email }}" aria-label="{{ __('Correo electrónico') }}" required><input name="phone" value="{{ $user->phone }}" aria-label="{{ __('Teléfono') }}"><input name="birth_date" type="date" value="{{ $user->birth_date?->format('Y-m-d') }}" aria-label="{{ __('Fecha de nacimiento') }}"><input name="address" value="{{ $user->address }}" aria-label="{{ __('Dirección') }}"><textarea name="admin_notes" rows="2" aria-label="{{ __('Notas internas') }}">{{ $user->admin_notes }}</textarea><input name="password" type="password" minlength="12" maxlength="72" autocomplete="new-password" placeholder="{{ __('Nueva contraseña (opcional)') }}" aria-label="{{ __('Nueva contraseña') }}"><input name="password_confirmation" type="password" minlength="12" maxlength="72" autocomplete="new-password" placeholder="{{ __('Repite la nueva contraseña') }}" aria-label="{{ __('Repite la nueva contraseña') }}"><label><input type="checkbox" name="is_admin" value="1" @checked($user->is_admin)> {{ __('Administrador') }}</label><button class="button button-small" type="submit">{{ __('Guardar ficha') }}</button></form></details><form method="POST" action="{{ route('admin.users.destroy', $user) }}" data-confirm-delete="{{ __('¿Seguro que quieres eliminar este registro?') }}"><input type="hidden" name="_admin_tab" value="admin-users">@csrf @method('DELETE')<button class="text-link danger-link" type="submit">{{ __('Eliminar') }}</button></form></td>
                            </tr>
                        @empty
