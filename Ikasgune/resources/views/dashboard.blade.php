    ...
    <section class="container dashboard-section" aria-labelledby="dashboard-title">
        ...
        <div class="auth-card account-card">
            <div class="account-details">
                <div class="account-heading"><span class="account-avatar" aria-hidden="true">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span><div><p class="eyebrow">PERFIL IKASGUNE</p><h2>Tu cuenta</h2></div></div>
                <dl><div><dt>Nombre</dt><dd>{{ auth()->user()->name }}</dd></div><div><dt>Correo electrónico</dt><dd>{{ auth()->user()->email }}</dd></div></dl>
            </div>
            <div class="account-actions"><a class="button" href="{{ route('courses.mine') }}">Mis cursos</a><a class="text-link" href="{{ route('courses.index') }}">Ver catálogo →</a></div>
        </div>
